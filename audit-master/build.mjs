...
const stale = [
  ...
  ["outreach.subject", o.subject, "Irma - I reviewed Bella Med Spa's patient journey"],
...
/**
 * The email gives credit before criticism, names ONE leak, and paraphrases it
 * rather than quoting the report's evidence in full. That last point matters:
 * the `observed` field IS the finding, and an email that hands it over leaves
 * no reason to open the review.
 *
 * `shortName` is used throughout - the full legal name reads badly in a
 * possessive ("Bella Med Spa & Aesthetics's").
 */
const name = p.shortName || p.name;
const bodyLines = [
  `Hi ${o.firstName},`,
  `I spent some time reviewing ${name}'s website on desktop and mobile.`,
  o.credit
    ? `The site is polished, and you are already doing several things better than most practices - particularly ${o.credit}.`
    : `The site is polished, and you are already doing several things better than most practices.`,
  `However, I found three places where a first-time premium patient may hesitate before completing a booking.`,
  `The first appears immediately: ${leak.title.charAt(0).toLowerCase()}${leak.title.slice(1)}.`,
  `I marked this and two other patient-journey leaks in a short private review:`,
];

const closing = "It takes about three minutes to read. There is no form to complete and no obligation - these observations are yours to use either way.";
...
${bodyLines.map((l) => `          <p style="margin:0 0 18px;">${esc(l)}</p>`).join("\n")}
          <p style="margin:0 0 14px;">
            <a href="${esc(o.auditUrl)}" style="display:inline-block;background:#0f2647;color:#ffffff;text-decoration:none;padding:14px 24px;font-weight:600;">View ${esc(name)}'s private website review</a>
          </p>
          <p style="margin:0 0 26px;font-size:14px;">
            <a href="${esc(o.auditUrl)}" style="color:#6b6f76;">${esc(o.auditUrl)}</a>
          </p>
